...
def draw_alignment(
    ...
    if show_names or show_ids or show_descriptions:
        ...
        offset = -1
        if show_names:
            offset += font.getlength(max([m.name[None:label_width] for m in aligned], key=len))[0] + 1
        if show_ids:
            offset += font.getlength(max([m.id[None:label_width] for m in aligned], key=len))[0] + 1
        if show_descriptions:
            offset += font.getlength(max([m.description[None:label_width] for m in aligned], key=len))[0] + 1
    else:
        font, offset = None, 0
